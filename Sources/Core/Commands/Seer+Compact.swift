...
extension Seer {
    ...
    struct CompactResult {
        ...
        /// to the Totem source document for exact span attribution.
    ...
    ///   - request: The `SeerRequest` with owner information for logging.
    ...
    /// (TotemContextStore) — the ONE marker that survives the search proto
    /// (group ids/tags/names are dropped by TotemPartitionResult), so it is
    ...
    nonisolated func compact(messages: [[String: Any]],
        ...
        if totalPartitionChars <= Seer.verbatimContextThreshold {
            ...
            **Perspectives From Others:**
            ...
            """
            logger.debug("Compact", "Verbatim context (\(totalPartitionChars) chars ≤ \(Seer.verbatimContextThreshold))", service: .seer, request: request)
        ...
        **PERSPECTIVES FROM OTHERS (NOT user owned):**
        ...
        You are preparing a briefing for an AI named Seer who is about to respond to a user. Your output is injected directly into Seer's system prompt so she can respond naturally and specifically.
        ...
        **Bonnie's Past Actions:** Sources from "BONNIE'S PAST TOOL ACTIONS". These are the assistant's own earlier tool runs — background about how similar requests were handled, NOT the user's prose and NOT the subject of the reply. Summarize only what helps the current request.
        ...
        logger.debug("Compact", "Output: \(output.prefix(500)), Content size: \(content.count), Output size: \(output.count)", service: .seer, request: request)
