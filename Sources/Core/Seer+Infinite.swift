...
extension Seer {
    ...
    ///
    /// Groups are sourced from Totem via `fanoutLibrary`; billing metrics come from
    ...
    nonisolated func leaderboard(page: Int = 1, pageSize: Int = 20) async -> (entries: [InfiniteGroupEntry], total: Int) {
        ...
        let rawMetrics: [(group: Seer.Group, metrics: GroupMetrics)] = publicGroups.map { group in
        ...
        var scored: [(group: Seer.Group, score: Double, metrics: GroupMetrics)] = rawMetrics.map { item in
            let score =
    ...
    nonisolated func searchGroups(query: String, limit: Int = 20) async -> [Seer.Group] {
        ...
        var matched: [(group: Seer.Group, score: Double)] = []
