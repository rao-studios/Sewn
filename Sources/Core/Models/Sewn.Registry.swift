...
struct SewnRegistry: Codable {
    ...
    /// that existed before Thread became the source of truth. Old serialised registries
    /// won't crash on decode — the large fields are simply skipped.
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        documentStats         = try c.decodeIfPresent([DocumentID: Sewn.DocumentStats].self, forKey: .documentStats)       ?? [:]
...
extension SewnRegistry {
    ...
    mutating func addPerformance(_ updates: [DocumentID: Sewn.DocumentStats]) {
...
// MARK: - Access (kept for Modify route return types and Sewn.Group model)
