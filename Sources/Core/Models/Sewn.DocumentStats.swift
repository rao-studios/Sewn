...
extension Sewn {
    ...
    ///
    /// `DocumentStats` is decoupled from `Sewn.Document` so the core document model
    /// stays lean. Stats are keyed by the same `DocumentID` and stored in
    /// `SewnRegistry.documentStats`, following the same registration, removal,
