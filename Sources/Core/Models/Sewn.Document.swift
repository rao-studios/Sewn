...
extension Sewn {
    ...
    /// Retrieve the `Sewn.Document` for a `DocumentID`.
    /// Returns the in-memory cached copy if available; falls back to disk.
    nonisolated func document(for id: DocumentID) -> Sewn.Document? {
    ...
    /// Retrieve the conversation `Sewn.Document` for a `DocumentID` via `FilePersistence`.
    /// - Parameter id: The `DocumentID`.
    /// - Returns: The `Sewn.Document` object.
    func conversationDocument(for id: DocumentID) -> Sewn.Document? {
