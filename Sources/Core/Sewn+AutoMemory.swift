...
extension Sewn {
    ...
    ///   - request: The `SewnRequest` carrying owner metadata.
    ...
    func autoMemorize(
        ...
        // 4. Compute document ID; no registry-based dedup (Thread is the source of truth).
        let documentId = computeHash(from: summaryTexts)

        // 5. Generate tags (embedding is handled by the Thread node at index time).
