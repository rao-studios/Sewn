...
actor EmbeddingModelProvider: EmbeddingProviding {
    ...
    init(logger: Logger) {
        self.logger = logger
        // Embeddings are Mistral-shaped (mistral-embed) — pin the base so the
        // globalLLM default (Tinker) can never swallow these requests.
        self.network = NetworkService(logger: logger, base: .mistral)
    }
