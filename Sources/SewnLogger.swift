...
/// preserving order and preventing dropped events.
///
/// Uses a shared singleton so all `SewnLogger` instances
...
/// logger.debug("Searching...", service: .sewn)
///
/// // With request tracing via SewnRequest:
/// logger.debug("Searching...", service: .sewn, request: sewnRequest)
///
...
struct SewnLogger {
    ...
    func externallyLog(
        ...
        if let requestID = sewn?.requestID, !requestID.isEmpty { entry["requestId"] = requestID }
        if let ownerId = sewn?.ownerId, !ownerId.isEmpty { entry["ownerId"] = ownerId }
    ...
    func debug(_ label: String? = nil,
        ...
        externallyLog(name: label, message: message().description, eventType: .debug, externalOnly: externalOnly, sewn: request, service: service, flow: flow)
        then?()
    }
    ...
    func info(_ label: String? = nil,
        ...
        externallyLog(name: label, message: message().description, eventType: .info, externalOnly: externalOnly, sewn: request, service: service, flow: flow)
        then?()
    }
    ...
    func warning(label: String? = nil,
        ...
        externallyLog(name: label, message: message().description, eventType: .warning, sewn: request, service: service, flow: flow)
        then?()
    }
    ...
    func error(_ label: String? = nil,
        ...
        externallyLog(name: label, message: message().description, eventType: .error, sewn: request, service: service, flow: flow)
        then?()
    }
