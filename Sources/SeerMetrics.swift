...
/// Central registry of Prometheus-backed metrics for the Seer server.
///
/// Metrics are backed by `PrometheusMetricsFactory` (bootstrapped in `SeerServer.setupApplication`).
...
enum SeerMetrics {
    ...
    static let searchTotal = Counter(label: "seer.search.total")
    /// End-to-end table search duration (HNSW/PQ scan only, excludes embedding time).
    static let searchDuration = Timer(label: "seer.search.duration")

    // MARK: - Chat pipeline (pre-stream legs + stream health)
    /// Retrieval leg of a chat turn (Totem fan-out or local search, end to end).
    static let totemSearchDuration = Timer(label: "seer.chat.search_duration")
    /// Context-compaction leg (verbatim injection or LLM briefing).
    static let compactDuration = Timer(label: "seer.chat.compact_duration")
    /// Request receipt → first visible streamed token.
    static let chatTTFT = Timer(label: "seer.chat.ttft")
    /// First visible token → stream completion.
    static let chatStreamDuration = Timer(label: "seer.chat.stream_duration")

    // MARK: - Realtime route (WebSocket chat + interleaved TTS)
    /// Total realtime turns started.
    static let realtimeTurns = Counter(label: "seer.realtime.turns_total")
    /// turn.start → first visible token (either phase).
    static let realtimeFirstToken = Timer(label: "seer.realtime.first_token")
    /// turn.start → first PCM frame on the socket.
    static let realtimeFirstAudio = Timer(label: "seer.realtime.first_audio")
    /// Gap between the opening finishing and retrieval becoming available —
    /// the audible seam the opening pass exists to cover.
    static let realtimeRetrievalWait = Timer(label: "seer.realtime.retrieval_wait")
    /// Turns whose TTS lane failed (text continued, audio stopped).
    static let realtimeTTSFailures = Counter(label: "seer.realtime.tts_failures_total")
    /// Turns whose retrieval pipeline failed (degraded grounded pass).
    static let realtimeRetrievalFailures = Counter(label: "seer.realtime.retrieval_failures_total")

    // MARK: - HNSW graph traversal (approximate last-seen values per query)
    /// Upper-layer hops taken during the greedy graph walk.
    static let hnswUpperHops = Gauge(label: "seer.hnsw.upper_layer_hops")
    /// Candidates examined at layer 0 (ef_search budget consumed).
    static let hnswLayer0Explored = Gauge(label: "seer.hnsw.layer0_explored")
    /// Effective ef ceiling used for the last base-layer beam search: max(k, efSearch).
    static let hnswEfUsed = Gauge(label: "seer.hnsw.ef_used")
    /// Candidate set size before owner/access filtering.
    static let hnswCandidates = Gauge(label: "seer.hnsw.candidates_before_filter")
    /// Live node count in the global HNSW graph after each search.
    static let hnswNodes = Gauge(label: "seer.hnsw.nodes_total")

    // MARK: - HNSW adaptive efSearch
    /// Current stored efSearch value after adaptive calibration (the floor, not max(k, efSearch)).
    static let hnswEfSearch = Gauge(label: "seer.hnsw.ef_search")
    /// EMA of layer-0 exploration depth driving the efSearch adaptation target.
    static let hnswEmaExplored = Gauge(label: "seer.hnsw.ema_explored")
    /// Total times efSearch changed value due to adaptation (not just every call to adaptEf).
    static let hnswEfAdaptations = Counter(label: "seer.hnsw.ef_adaptations_total")

    // MARK: - HNSW graph health (periodic snapshots)
    /// Maximum layer reached in the global HNSW hierarchy.
    static let hnswMaxLevel = Gauge(label: "seer.hnsw.max_level")
    /// Live (non-deleted) node count in the global HNSW graph.
    static let hnswLiveNodes = Gauge(label: "seer.hnsw.live_nodes")
    /// Soft-deleted node count pending the next compaction.
    static let hnswDeletedNodes = Gauge(label: "seer.hnsw.deleted_nodes")
    /// Average number of layer-0 neighbours per live node.
    static let hnswAvgLayer0Degree = Gauge(label: "seer.hnsw.avg_layer0_degree")
    /// Total compaction runs that removed at least one deleted node.
    static let hnswCompactions = Counter(label: "seer.hnsw.compactions_total")

    // MARK: - Product Quantization training
    /// Wall-clock time to train a PQ codebook for one document (milliseconds).
    static let pqTrainDuration = Timer(label: "seer.pq.train_duration")

    // MARK: - Index lifecycle
    /// Total number of indexed documents (keys in PartitionTable).
    static let indexDocuments = Gauge(label: "seer.index.documents_total")
    ...
    //
    // Dashboard recipe: overlay `seer_batch_phase_duration_ms{phase="phase3_index"}`
    ...
    static let batchPhaseDuration = Timer(label: "seer.batch.phase_duration")
    ...
    static let batchDocuments = Counter(label: "seer.batch.documents_total")
    ...
    static let indexQueueDepth = Gauge(label: "seer.index.queue_depth")
    ...
    static let indexQueueItems = Gauge(label: "seer.index.queue_items")
