...
actor SeerGRPCServer {
    private var serverTask: Task<Void, Error>?

    @discardableResult
    func start(registry: RegistryMutator, nodeId: UUID, grpcPort: Int, sessionManager: TotemSessionManager, logger: SeerLogger) -> TotemSessionManager {
        let service = TotemRegistrationServiceImpl(registry: registry, mothershipId: nodeId, sessionManager: sessionManager, logger: SeerConduitLogger(base: logger))
        serverTask = Task {
            let server = GRPCServer(
                transport: .http2NIOPosix(
                    ...
                    config: .defaults {
                        ...
                        // Detect dead Totem connections from this side too
            ...
            logger.info("SeerGRPCServer", "gRPC server listening on port \(grpcPort)", service: .startup)
