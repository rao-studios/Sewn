...
/// Wipes all Seer persistence files written by a test run.
/// Safe to call before `Seer()` is constructed — does not touch `node-id`,
/// so shard-scoped node identity stays stable across the test session.
func wipeSeerPersistenceFiles() {
...
extension Logger {
    static var test: Logger {
        var logger = Logger(label: "test-seer")
...
extension Seer.Document {
    static func test(
        ...
        Seer.Document(id: id, url: URL(string: "https://example.com")!, ownerId: ownerId)
    }
}
...
extension Seer.Group {
    static func test(
        ...
        ownerId: String = "test-owner",
        metadata: Seer.Group.Metadata? = nil
    ) -> Seer.Group {
        Seer.Group(id: id, label: label, ownerId: ownerId, documents: [], metadata: metadata)
    }
}

// MARK: - Seer.Group.Metadata

extension Seer.Group.Metadata {
    static func test(
        description: String? = "A test group description",
        tags: [String] = ["swift", "test"]
    ) -> Seer.Group.Metadata {
        Seer.Group.Metadata(description: description, tags: tags)
    }
}
...
/// registerBatchEmbeddingsRoute(app, seer,
///                              modelProvider: ...,
