...
final class Flow2_GitaWalletTests: XCTestCase {
    ...
    private func makePartition(documentId: String, ownerId: String, text: String) -> Seer.Partition {
        Seer.Partition.test(documentId: documentId, text: text, ownerId: ownerId)
    }
    ...
    private func pricedContribution(spenderId: String? = "alice") -> Gita.Contribution {
        ...
        let request      = spenderId.map { SeerRequest(ownerId: $0, group: nil, aggregate: nil, scope: nil, requestID: nil) }
    ...
    func testCreditExchangePayoutsMatchOwnerEarnings() {
        ...
        for owner in priced.owners where owner.earning > 0 {
            let ownerKey = owner.ownerId ?? owner.totemId
    ...
    func testSelfReferentialOwnerEarnsZero() {
        ...
        let contribution = gita.royalty(for: partitions)
        let ledger       = Gita.TokenLedger.test()
        let request      = SeerRequest(ownerId: "bob", group: nil, aggregate: nil, scope: nil, requestID: nil)
