...
final class Flow3_SinatraParkAlignmentTests: XCTestCase {
    ...
    private func uniqueRequest() -> SeerRequest {
        SeerRequest.test(ownerId: testOwnerId)
    }
    ...
    func testParkStoresAllPassedPartitions() {
        ...
        let scored: [(score: Float, partition: Seer.Partition)] = (0..<5).map { i in
    ...
    func testParkWithFewerThanThreePartitionsStoresAll() {
        ...
        for count in [1, 2, 4, 7] {
            ...
            let request = SeerRequest.test(ownerId: iterationOwnerId)
            let freshSinatra = Sinatra(logger: .test)
            let scored: [(score: Float, partition: Seer.Partition)] = (0..<count).map { i in
                (score: Float(i) * 0.1, partition: .test(id: "p\(i)"))
            }
            freshSinatra.park(data: scored, forQuery: [], request: request)

            let owner = SeerRegistry.Owner(id: iterationOwnerId)
    ...
    func testParkStoresScoreAsDistance() {
        let request = uniqueRequest()
        let scored: [(score: Float, partition: Seer.Partition)] = [
    ...
    func testParkPartitionIdsMatchInput() {
        ...
        let scored = ids.map { id in
            (score: Float(0.1), partition: Seer.Partition.test(id: id))
        }
    ...
    func testParkIsolatedAcrossOwners() {
        ...
        let requestA = SeerRequest.test(ownerId: ownerAId)
        let requestB = SeerRequest.test(ownerId: ownerBId)
        ...
        let ownerA = SeerRegistry.Owner(id: ownerAId)
        let ownerB = SeerRegistry.Owner(id: ownerBId)
    ...
    func testLocalScoreWinsOverPeerScoreForSharedPartition() {
        ...
        let allPartitions: [Seer.Partition] = [
            .test(id: "shared"),
            .test(id: "local-only"),
            .test(id: "peer-only"),
        ]

        let allScored: [(score: Float, partition: Seer.Partition)] = allPartitions.compactMap { p in
    ...
    func testLocalOnlyPartitionsAllReceiveScores() {
        ...
        let allPartitions = ["a", "b", "c"].map { Seer.Partition.test(id: $0) }
    ...
    func testExpandedScoreMapKeepsBestScoreWhenPartitionAppearsInMultipleVariants() {
        ...
        let variantResults: [(score: Float, partition: Seer.Partition)] = [
    ...
    func testExpandedScoreMapAllUniquePartitionsPreserved() {
        let variantResults: [(score: Float, partition: Seer.Partition)] = (0..<5).map { i in
            (score: Float(i) * 0.1, partition: .test(id: "p\(i)"))
        }
    ...
    func testExpandedUniquePartitionsAllGetScores() {
        ...
        let allResults: [(score: Float, partition: Seer.Partition)] = [
        ...
        let uniqueScored: [(score: Float, partition: Seer.Partition)] = uniquePartitions.compactMap { p in
    ...
    func testConsumedIdFilterDoesNotAffectOtherOwners() {
        ...
        let requestB = SeerRequest.test(ownerId: UUID().uuidString)
        ...
        let ownerA = SeerRegistry.Owner(id: requestA.ownerId)
        let ownerB = SeerRegistry.Owner(id: requestB.ownerId)
    ...
    func testParkIndicesIsolatedAcrossOwners() {
        let requestA = SeerRequest.test(ownerId: UUID().uuidString)
        let requestB = SeerRequest.test(ownerId: UUID().uuidString)
