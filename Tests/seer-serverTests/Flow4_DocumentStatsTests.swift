...
//  seer-serverTests
//
//  Tests for `Seer.DocumentStats` and `SeerRegistry` performance accumulation
...
//  3. `SeerRegistry.addPerformance` — additive (not replace) across two calls for
...
final class Flow4_DocumentStatsTests: XCTestCase {
    ...
    func testPartitionSentimentDefaultsToNeutralPrior() {
        let ps = Seer.DocumentStats.PartitionSentiment()
    ...
    func testPartitionSentimentAverageWithOneReading() {
        var ps = Seer.DocumentStats.PartitionSentiment()
        ps.retrievalCount = 1
        ps.sentimentSum   = 0.8
    ...
    func testPartitionSentimentEncodeDecode() throws {
        ...
        let decoded = try decoder.decode(Seer.DocumentStats.PartitionSentiment.self, from: data)
    ...
    func testPartitionSentimentDecodesFromEmptyObjectWithDefaults() throws {
        ...
        let ps = try JSONDecoder().decode(Seer.DocumentStats.PartitionSentiment.self, from: json)
        XCTAssertEqual(ps.retrievalCount, 0)
        XCTAssertEqual(ps.sentimentSum, 0.0, accuracy: 1e-9)
        XCTAssertEqual(ps.averageSentiment, 0.5, accuracy: 1e-9)
    }
    ...
    func testDocumentStatsDefaultHasEmptyPartitionSentiments() {
        let stats = Seer.DocumentStats(id: "doc1")
    ...
    func testDocumentStatsEncodesAndDecodesPartitionSentiments() throws {
        var stats = Seer.DocumentStats(id: "doc1")
        var ps = Seer.DocumentStats.PartitionSentiment()
        ps.retrievalCount = 2
        ps.sentimentSum   = 1.4
        ...
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .secondsSince1970
        let decoded = try decoder.decode(Seer.DocumentStats.self, from: data)
    ...
    func testDocumentStatsDecodesFromLegacyJsonWithoutPartitionSentiments() throws {
        ...
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .secondsSince1970
        let stats = try decoder.decode(Seer.DocumentStats.self, from: json)
        XCTAssertTrue(stats.partitionSentiments.isEmpty,
            "Legacy JSON without partition_sentiments must decode to an empty map")
        XCTAssertEqual(stats.retrievalCount, 3)
    }

    // MARK: - SeerRegistry.addPerformance — additive accumulation
    ...
    func testAddPerformanceAccumulatesRetrievalCountAcrossTwoCycles() {
        ...
        var update1 = Seer.DocumentStats(id: "doc1")
        ...
        var update2 = Seer.DocumentStats(id: "doc1")
    ...
    func testAddPerformancePreservesTotalEarned() {
        ...
        registry.documentStats["doc1"] = Seer.DocumentStats(id: "doc1", totalEarned: 50)

        var update = Seer.DocumentStats(id: "doc1")
    ...
    func testAddPerformanceMergesPartitionSentimentsAdditively() {
        ...
        var ps1 = Seer.DocumentStats.PartitionSentiment()
        ps1.retrievalCount = 2; ps1.sentimentSum = 1.6
        var update1 = Seer.DocumentStats(id: "doc1")
        update1.partitionSentiments["pX"] = ps1
        registry.addPerformance(["doc1": update1])

        var ps2 = Seer.DocumentStats.PartitionSentiment()
    ...
    func testAddPerformanceIsolatedAcrossDocuments() {
        ...
        var updateA = Seer.DocumentStats(id: "docA")
        updateA.retrievalCount = 2
        var updateB = Seer.DocumentStats(id: "docB")
    ...
    func testGenerateFeatureVectorUsesPartitionSpecificSentiment() {
        ...
        var docStats = Seer.DocumentStats(id: "doc1")
    ...
    func testGenerateFeatureVectorDifferentPartitionsGetDifferentSentimentFeatures() {
        ...
        var psPositive = Seer.DocumentStats.PartitionSentiment()
        psPositive.retrievalCount = 2; psPositive.sentimentSum = 1.8  // avg = 0.9

        var psNegative = Seer.DocumentStats.PartitionSentiment()
    ...
    func testSeerFlowFrankServiceName() {
        let flow = SeerLogger.SeerFlow.frank(partitionId: "abc123")
        XCTAssertEqual(flow.serviceName, "Flow: Frank")
    }

    func testSeerFlowFrankExposesPartitionId() {
        let flow = SeerLogger.SeerFlow.frank(partitionId: "my-partition")
        XCTAssertEqual(flow.partitionId, "my-partition")
    }

    func testSeerFlowFrankDocumentIdIsNil() {
        let flow = SeerLogger.SeerFlow.frank(partitionId: "p")
        XCTAssertNil(flow.documentId,
            "frank flow must not expose a documentId — it carries partitionId instead")
    }

    func testSeerFlowChatAndEmbedPartitionIdIsNil() {
        XCTAssertNil(SeerLogger.SeerFlow.chat.partitionId)
        XCTAssertNil(SeerLogger.SeerFlow.embed(documentId: "doc1").partitionId)
    }

    func testSeerFlowEmbedDocumentIdIsCorrect() {
        let flow = SeerLogger.SeerFlow.embed(documentId: "doc-xyz")
