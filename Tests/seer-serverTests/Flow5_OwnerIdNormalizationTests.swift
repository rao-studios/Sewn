...
//  "0455D67E-..."). The fix normalizes ownerId to lowercase at Seer.Partition
...
//
//  Sections
//    1. Seer.Partition — ownerId is always lowercase at init and decode time
...
final class Flow5_OwnerIdNormalizationTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("seer-flow10-\(UUID().uuidString)")
    ...
    // =========================================================================
    // MARK: - Section 1: Seer.Partition — ownerId normalization at construction
    // =========================================================================

    func testPartitionInitLowercasesOwnerId() {
        let p = Seer.Partition.test(ownerId: "0455D67E-6B13-41CF-ACC0-4F0762C76A0B")
    ...
    func testPartitionInitAlreadyLowercaseIsUnchanged() {
        ...
        let p = Seer.Partition.test(ownerId: lower)
    ...
    func testPartitionInitMixedCaseIsLowercased() {
        let p = Seer.Partition.test(ownerId: "Alice")
    ...
    func testPartitionDecodeLowercasesOwnerId() throws {
        ...
        let p = try JSONDecoder().decode(Seer.Partition.self, from: json)
        XCTAssertEqual(p.ownerId, "0455d67e-6b13-41cf-acc0-4f0762c76a0b",
    ...
    func testPartitionDecodeLowerAndUpperProduceSameOwnerId() throws {
        func decode(_ ownerId: String) throws -> Seer.Partition {
            let json = """
            {"id":"p","document_id":"d","url":"https://example.com","embedding":[],"text":"t","owner_id":"\(ownerId)"}
            """.data(using: .utf8)!
            return try JSONDecoder().decode(Seer.Partition.self, from: json)
        }
    ...
    func testRoyaltyMixedCaseOwnerIdsProduceSingleEntry() {
        ...
        let partitions = [
            Seer.Partition.test(id: "p1", documentId: "doc1", text: "hello world foo bar", ownerId: canonical),
            Seer.Partition.test(id: "p2", documentId: "doc2", text: "one two",             ownerId: uppercase),
        ]
    ...
    func testRoyaltyAllUpperCaseOwnerIdFolded() {
        let partitions = [
            Seer.Partition.test(id: "p1", documentId: "d1", text: "word1 word2", ownerId: "ALICE"),
            Seer.Partition.test(id: "p2", documentId: "d2", text: "word3",       ownerId: "alice"),
        ]
    ...
    func testRoyaltyDistinctOwnersUnaffected() {
        let partitions = [
            Seer.Partition.test(id: "p1", documentId: "d1", text: "hello world", ownerId: "alice"),
            Seer.Partition.test(id: "p2", documentId: "d2", text: "foo bar baz", ownerId: "bob"),
        ]
    ...
    func testRoyaltyMixedCaseSumsToOne() {
        ...
        let partitions = [
            Seer.Partition.test(id: "p1", documentId: "d1", text: "a b c",     ownerId: "Owner-X"),
            Seer.Partition.test(id: "p2", documentId: "d2", text: "d e",       ownerId: "OWNER-X"),
            Seer.Partition.test(id: "p3", documentId: "d3", text: "f g h i j", ownerId: "owner-x"),
        ]
