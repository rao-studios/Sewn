...
final class RealtimeOpeningPromptTests: XCTestCase {
    ...
    func testOpeningPromptFramesSewnAsActor() {
...
final class RealtimeWireTests: XCTestCase {
    ...
    func testTurnStartDecodesEmbeddedChatRequest() throws {
        let json = """
        {"type":"turn.start","tts":{"voice_id":"fr_marie_calm"},"request":{"messages":[{"role":"user","content":"hey"}],"stream":true,"sewn":{"owner_id":"o1","personal_thread_id":"t1"}}}
        """
        ...
        XCTAssertEqual(start.request.sewn.personalThreadId, "t1")
...
final class CompactVerbatimTests: XCTestCase {
    ...
    func testSmallRetrievalInjectsVerbatim() async throws {
        let sewn = Sewn()
        let partitions = [
            Sewn.Partition.test(documentId: "doc-a", text: "Alpha fact lives here.", ownerId: "owner-1"),
            Sewn.Partition.test(documentId: "doc-b", text: "Beta fact lives here.", ownerId: "owner-1"),
        ]
        let result = try await sewn.compact(
            messages: [],
    ...
    func testThresholdCountsPartitionCharacters() {
        ...
        let under = String(repeating: "a", count: Sewn.verbatimContextThreshold)
        let over = under + "a"
        XCTAssertLessThanOrEqual(under.count, Sewn.verbatimContextThreshold)
        XCTAssertGreaterThan(over.count, Sewn.verbatimContextThreshold)
    }
}
...
final class BonnieClientFramingTests: XCTestCase {
    ...
    func testBonnieToolPartitionsGetTheirOwnVerbatimTier() async throws {
        let sewn = Sewn()
        let partitions = [
            Sewn.Partition.test(documentId: "doc-a", text: "Chapter notes live here.", ownerId: "owner-1"),
            Sewn.Partition.test(documentId: "bonnie-tool-123", text: "replace_symbol — for: add pink\nEdited Paper.swift", ownerId: "owner-1"),
        ]
        let result = try await sewn.compact(
            messages: [],
            partitions: partitions,
    ...
    func testClassicClientKeepsToolDepositsInDocuments() async throws {
        let sewn = Sewn()
        let partitions = [
            Sewn.Partition.test(documentId: "bonnie-tool-123", text: "tool output text", ownerId: "owner-1"),
        ]
        let result = try await sewn.compact(
            messages: [],
            partitions: partitions,
    ...
    func testContextUsageGuideFraming() {
        let bonnie = Sewn.contextUsageGuide(bonnieClient: true, citationProtocol: "CITE-RULES")
        ...
        let classic = Sewn.contextUsageGuide(bonnieClient: false, citationProtocol: "CITE-RULES")
