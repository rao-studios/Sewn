...
//
//  These tests cover the pure-logic layers of Sewn+QueryExpander.swift without
...
final class Flow6_QueryExpansionTests: XCTestCase {
    ...
    func testAllIncludesOriginalFirst() {
        let expansion = Sewn.QueryExpansion(original: "what is sewn", variants: ["a", "b"])
        XCTAssertEqual(expansion.all.first, "what is sewn",
    ...
    func testAllContainsAllVariants() {
        ...
        let expansion = Sewn.QueryExpansion(original: "original", variants: variants)
        for v in variants {
    ...
    func testAllCountIsOriginalPlusVariants() {
        ...
        let expansion = Sewn.QueryExpansion(original: "orig", variants: variants)
    ...
    func testAllWithEmptyVariantsIsJustOriginal() {
        let expansion = Sewn.QueryExpansion(original: "solo query", variants: [])
    ...
    func testParsingExcludesVariantIdenticalToOriginal() {
        let original = "what is sewn"
        let raw = "what is sewn\nother query\nanother angle"
        let variants = parseVariants(from: raw, original: original)
    ...
    private func deduplicatePartitions(_ partitions: [Sewn.Partition]) -> [Sewn.Partition] {
        var seen = Set<String>()
        return partitions.filter { seen.insert($0.id).inserted }
    }

    func testDeduplicationRemovesDuplicateIds() {
        let p1 = Sewn.Partition.test(id: "dup-id", documentId: "doc1")
        let p2 = Sewn.Partition.test(id: "dup-id", documentId: "doc1")  // same id
        let p3 = Sewn.Partition.test(id: "unique-id", documentId: "doc2")
    ...
    func testDeduplicationFirstOccurrenceWins() {
        let first  = Sewn.Partition.test(id: "shared", documentId: "docA", text: "first text")
        let second = Sewn.Partition.test(id: "shared", documentId: "docA", text: "second text")
    ...
    func testDeduplicationPreservesAllUniquePartitions() {
        let partitions = (0..<5).map {
            Sewn.Partition.test(id: "p\($0)", documentId: "doc\($0)")
        }
    ...
    func testDeduplicationAllSameIdRetainsOne() {
        let partitions = (0..<4).map {
            Sewn.Partition.test(id: "same-id", documentId: "doc", text: "text-\($0)")
        }
        let unique = deduplicatePartitions(partitions)
    ...
    func testExpansionAlwaysProducesAtLeastOneQuery() {
        let expansionFromLLMFailure = Sewn.QueryExpansion(original: "any query", variants: [])
    ...
    func testExpansionOriginalIsNeverModified() {
        ...
        let expansion = Sewn.QueryExpansion(original: original, variants: ["v1"])
