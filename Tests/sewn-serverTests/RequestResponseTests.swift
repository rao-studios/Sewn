...
final class RequestResponseTests: XCTestCase {
    ...
    func testEmbeddingRequestWithStringInput() throws {
        ...
        XCTAssertEqual(request.inputs.flatMap(\.values), ["Hello world"])
    ...
    func testEmbeddingRequestWithArrayInput() throws {
        ...
        XCTAssertEqual(request.inputs.flatMap(\.values), ["First text", "Second text", "Third text"])
        XCTAssertEqual(request.model, "embedding-model")
    }

    func testEmbeddingRequestMetadataOmittedIsNil() throws {
        let jsonData = """
        {
            "inputs": ["test"],
            "sewn": {"owner_id": "owner"}
        ...
        let request = try JSONDecoder().decode(EmbeddingRequest.self, from: jsonData)
        XCTAssertNil(request.metadata, "omitted metadata must decode as nil")
    }

    func testEmbeddingRequestDecodesMetadataArray() throws {
        let p0 = Data("doc0".utf8)
        let jsonData = """
        {
            "inputs": [["text0"], ["text1"]],
            "sewn": {"owner_id": "owner"},
            "metadata": ["\(p0.base64EncodedString())", null]
        }
        """.data(using: .utf8)!

        let request = try JSONDecoder().decode(EmbeddingRequest.self, from: jsonData)
        XCTAssertNotNil(request.metadata, "metadata array must be decoded")
        XCTAssertEqual(request.metadata?.count, 2, "metadata array must have one entry per input")
        XCTAssertEqual(request.metadata?[0], p0, "first entry must match p0 payload")
        XCTAssertNil(request.metadata?[1] ?? nil, "second entry must be nil")
    }
