...
final class Flow6_AutoMemoryTests: XCTestCase {
    ...
    private func messageCountThreshold(in policy: Sewn.AutoMemoryPolicy) -> Int? {
    ...
    private func policyFires(
        fired: Set<Sewn.AutoMemoryTrigger>,
        policy: Sewn.AutoMemoryPolicy
    ) -> Bool {
    ...
    func testDefaultPolicyIsEither() {
        let policy = Sewn.autoMemoryPolicy
    ...
    func testDefaultMessageCountThresholdIsSeven() {
        let threshold = messageCountThreshold(in: Sewn.autoMemoryPolicy)
    ...
    func testAutoMemoryGroupLabelIsMemory() {
        XCTAssertEqual(Sewn.autoMemoryGroupLabel, "Memory",
    ...
    func testAnyPolicyFiresOnMessageCountAlone() {
        let policy = Sewn.AutoMemoryPolicy.either
        XCTAssertTrue(
    ...
    func testAllPolicyDoesNotFireOnMessageCountAlone() {
        let policy = Sewn.AutoMemoryPolicy.all
        XCTAssertFalse(
            policyFires(fired: [.messageCount()], policy: policy),
    ...
    func testMessageCountOnlyPolicyIgnoresTopicChange() {
        let policy = Sewn.AutoMemoryPolicy.messageCountOnly
        XCTAssertFalse(
            policyFires(fired: [.topicChange], policy: policy),
    ...
    func testTopicChangeOnlyPolicyIgnoresMessageCount() {
        let policy = Sewn.AutoMemoryPolicy.topicChangeOnly
        XCTAssertFalse(
            policyFires(fired: [.messageCount()], policy: policy),
