...
//  sewn-serverTests
//
//  Local mode: a Sewn an app launched for itself answers only requests that
...
//  The primitives live in RaoStack now; these tests pin how Sewn uses them
//  through StackMode, its middleware and its /health route.
//
...
final class StackSecretTests: XCTestCase {
    ...
    private func single(_ secret: String) -> StackMode { .single(secret: secret, app: nil) }
    ...
    func testOnlyTheExactSecretMatches() {
        ...
        XCTAssertEqual(single("s3cret").admit(authority: "localhost", presented: "s3cret"), .admitted(nil))
        XCTAssertEqual(single("s3cret").admit(authority: "localhost", presented: "s3cres"), .badSecret)
        XCTAssertEqual(single("s3cret").admit(authority: "localhost", presented: nil), .badSecret)
    }

    func testHealthProvesTheSecretForANonce() {
        XCTAssertEqual(StackSecret.proof(nonce: KAT.nonce, secret: "s3cret"), KAT.proofForS3cret)

        let unchallenged = single("s").healthAnswer(nonce: nil, requestedApp: nil)
        XCTAssertEqual(unchallenged.stack, "proof")
        XCTAssertNil(unchallenged.proof)

        for malformed in ["zz", "", String(repeating: "ab", count: 100)] {
            let answer = single("s").healthAnswer(nonce: malformed, requestedApp: nil)
        ...
        XCTAssertNil(open.app)
        XCTAssertNil(open.contract)

        let uppercase = single("s3cret").healthAnswer(nonce: KAT.nonce.uppercased(), requestedApp: nil)
    ...
    func testOnlyLoopbackHostsAreAnswered() {
        for host in ["127.0.0.1:47080", "localhost:47080", "LOCALHOST", "[::1]:47080", "127.0.0.1"] {
            ...
            XCTAssertEqual(single("s").admit(authority: host, presented: "s"), .admitted(nil), host)
        }
        for host in ["evil.example:47080", "127.0.0.1.evil.example", "192.168.1.4:47080", "", "[::2]:1"] {
            ...
            XCTAssertEqual(single("s").admit(authority: host, presented: "s"), .notLoopback, host)
        }
        XCTAssertFalse(StackSecret.isLoopback(authority: nil))
        XCTAssertEqual(single("s").admit(authority: nil, presented: "s"), .notLoopback)
    }

    func testARebindingPageIsRefusedEvenWithTheSecret() {
        XCTAssertThrowsError(try single("s").admittedApp(authority: "evil.example:47080", presented: "s")) { error in
            XCTAssertEqual((error as? HTTPError)?.status, .misdirectedRequest)
        }
        XCTAssertThrowsError(try single("s").admittedApp(authority: "localhost", presented: "wrong")) { error in
            XCTAssertEqual((error as? HTTPError)?.status, .unauthorized)
        }
        XCTAssertNil(try StackMode.open.admittedApp(authority: "evil.example", presented: nil),
                     "no secret configured: nothing is refused")
    }

    /// The real middleware, the real /health route and the real context.
    ...
    func testHealthAnswersAChallengeWithoutTheSecret() async throws {
        try await app(single("s3cret")).test(.router) { client in
    ...
    func testWithoutTheSecretNothingButHealthAnswers() async throws {
        try await app(single("s")).test(.router) { client in
