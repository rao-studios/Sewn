...
// MARK: - Palette (warm editorial "Seer" language — light-only)
...
extension Color {
    ...
    static let seerGreen  = Color(red:  90/255, green: 150/255, blue:  95/255)
    static let seerLabel  = Color(red:  12/255, green:  12/255, blue:  12/255)
}
...
enum SeerMetrics {
    ...
    /// Minimum height for SeerButtonStyle buttons.
...
/// The Seer glyph — an eye in gold, hierarchical rendering.
struct SeerMark: View {
    var size: CGFloat = 22
    var body: some View {
    ...
    var body: some View {
        ZStack {
            ...
            Circle()
                .fill(
                    RadialGradient(
                        colors: [Color.seerGold.opacity(0.18), .clear],
                        center: .center,
                        startRadius: 0,
                        endRadius: iconSize
                    )
...
struct SeerEmblem: View {
    var iconSize: CGFloat = 56
    var body: some View {
        ZStack {
            SeerOrbitRings(iconSize: iconSize)
            SeerMark(size: iconSize * 0.6)
        }
